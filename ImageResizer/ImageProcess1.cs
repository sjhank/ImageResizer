    ...
    public class ImageProcess1
    {
        ...
        public void ResizeImages(string sourcePath, string destPath, double scale)
        {
            ...
            for (int i = 0; i < allFiles.Count; i++)
            {
                ...
                tasks[i] = ResizeImageAsync(destPath, scale, filePath);
            }
            Task.WaitAll(tasks);
        }

        private async Task ResizeImageAsync(string destPath, double scale, string filePath)
        {
            ...
            Bitmap processedImage = await Task.Run(async () => await processBitmapAsync((Bitmap)imgPhoto,
